...
import com.tokyonth.facedetector.R
import com.tokyonth.facedetector.utils.BitmapUtils
...
class Camera2(
    ...
    companion object {
        private const val TAG = "FaceCamera2"
    ...
    private val onTouchPreviewListener = OnTouchListener { view, event ->
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                val focusX =
                    (event.y / view.height.toFloat() * sensorArraySize.width.toFloat()).toInt()
                val focusY =
                    ((1 - event.x / view.width.toFloat()) * sensorArraySize.height.toFloat()).toInt()
            ...
            backgroundHandler?.post {
                ...
                try {
                    ...
                    val previewSize = Size(
                        ((textureView.parent as ViewGroup).width),
                        ((textureView.parent as ViewGroup).height)
                    )
                    val configureBitmap = BitmapUtils.configureBitmap(bytes, previewSize, cameraId)
                    if (modeCapture != ICamera.ModeCapture.IDLE) {
                        FileUtils.saveImageJPEGIntoMediaFolder(
                            context,
                            configureBitmap,
                            FileUtils.getNameImageDynamic()
                        ) {
                            countImagesSaved++
                            if (modeCapture != ICamera.ModeCapture.CAPTURE) {
                                takePictureCallbacks?.captureBurstSucceeded(
                                    configureBitmap,
            ...
            override fun onCaptureCompleted(
                ...
                if (request.tag == FOCUS_TAG) {
                    try {
                        ...
                        Log.e(FOCUS_TAG, "Focus failed onCaptureCompleted ${e.message}")
                    }
                }
            }
        }
    ...
    override fun openCamera(cameraFace: ICamera.CameraFace) {
        initBackgroundThread()

        val cameraManager = context.getSystemService(Context.CAMERA_SERVICE) as CameraManager
        cameraId = if (cameraFace == ICamera.CameraFace.BACK) {
            "0"
        } else {
            "1"
        }
        ...
        val previewSize = CameraUtils.calculationSize(
            mapConfig,
            CameraUtils.CalculationSize.PREVIEW
        )
    ...
    override fun switchCamera() {
        ...
        if (cameraId.contains("0")) {
            openCamera(ICamera.CameraFace.FRONT)
        } else {
            openCamera(ICamera.CameraFace.BACK)
        }
    }
    ...
    override fun captureBurstFreeHand(
    ...
    private fun previewCamera() {
        ...
        val imageSize = CameraUtils.calculationSize(mapConfig!!, CameraUtils.CalculationSize.IMAGE)
        ...
        imageReader =
            ImageReader.newInstance(imageSize.width, imageSize.height, ImageFormat.JPEG, 2)
        ...
        try {
            ...
            arrayRect.forEach {
                val meteringRectangle =
                    MeteringRectangle(it, MeteringRectangle.METERING_WEIGHT_MAX - 1)
            ...
            if (isMeteringAreaAFSupported()) {
                ...
                previewRequestBuilder?.set(
                    ...
                    arrayMeteringRectangle.toTypedArray()
                )
                previewRequestBuilder?.set(
                    CaptureRequest.CONTROL_AF_TRIGGER,
                ...
                if (rectFocus.centerY() < boundsArray[0].centerY() - DISTANCE_REDRAW ||
                    ...
                    boundsArray.forEach { boundsOnSensor ->
                        ...
                        if (cameraId.contains("1")) {// front camera
                            centerY = (1.0f - boundsOnSensor.centerX()
                                .toFloat() / sensorArraySize.width) *
                                    textureView.height
                            centerX = (1.0f - boundsOnSensor.centerY()
                                .toFloat() / sensorArraySize.height) *
                                    textureView.width
                            previewX = centerX
                            previewY = centerY
                            ratioFace = 1f
                            ...
                            centerY = ((boundsOnSensor.centerX()
                                .toFloat() * textureView.height.toFloat()) /
                                    sensorArraySize.width.toFloat())
                            centerX = ((1.0f - boundsOnSensor.centerY()
                                .toFloat() / sensorArraySize.height.toFloat()) *
                             ...
                             */
                            val plusX =
                                (centerX - (textureView.width / 2)) * (textureView.width.toFloat() /
                        ...
                        if (!isLandscape) {
                            widthFace =
                                (boundsOnSensor.width().toFloat() * textureView.height.toFloat() /
                                        sensorArraySize.width * ratioFace)
                            heightFace =
                                (boundsOnSensor.height().toFloat() * textureView.width.toFloat() /
