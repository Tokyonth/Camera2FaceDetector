...
class FaceBorderView(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    override fun onDraw(canvas: Canvas) {
        when (mode) {
            MODE_TOUCH_FOCUS -> {
                canvas.drawRect(
                    rectFTouch.left, rectFTouch.top, rectFTouch.right,
                    rectFTouch.bottom, paint
                )
                canvas.drawLine(
                    rectFTouch.left, (rectFTouch.top + rectFTouch.bottom) / 2,
                    rectFTouch.left + widthLine, (rectFTouch.top + rectFTouch.bottom) / 2, paint
                )
                canvas.drawLine(
                    rectFTouch.right, (rectFTouch.top + rectFTouch.bottom) / 2,
                    rectFTouch.right - widthLine, (rectFTouch.top + rectFTouch.bottom) / 2, paint
                )
                canvas.drawLine(
                    (rectFTouch.left + rectFTouch.right) / 2, rectFTouch.top,
                    (rectFTouch.left + rectFTouch.right) / 2, rectFTouch.top + widthLine, paint
                )
                canvas.drawLine(
                    (rectFTouch.left + rectFTouch.right) / 2, rectFTouch.bottom,
                ...
                if (listFaces.isNotEmpty()) {
                    for (i in 0..listFaces.lastIndex) {
                        canvas.drawRoundRect(
                            listRectF[i].left, listRectF[i].top,
                ...
                for (i in 0..list.lastIndex) {
                    animatorFaceDetection = ValueAnimator.ofPropertyValuesHolder(
                        PropertyValuesHolder.ofFloat(
                            propertyLeft, listRectF[i].left,
                            list[i].left
                        ),
                        PropertyValuesHolder.ofFloat(
                            propertyTop, listRectF[i].top,
                            list[i].top
                        ),
                        PropertyValuesHolder.ofFloat(
                            propertyRight, listRectF[i].right,
                            list[i].right
                        ),
                        PropertyValuesHolder.ofFloat(
                            propertyBottom, listRectF[i].bottom,
                            list[i].bottom
                        )
                    )
    ...
    fun touchTo(x: Float, y: Float) {
        ...
        animatorTouchFocus = ValueAnimator.ofPropertyValuesHolder(
            PropertyValuesHolder.ofFloat(
                propertyLeft, touchX - sizeRecFBefore,
                touchX - sizeRecFLate
            ),
            PropertyValuesHolder.ofFloat(
                propertyTop, touchY - sizeRecFBefore,
                touchY - sizeRecFLate
            ),
            PropertyValuesHolder.ofFloat(
                propertyRight, touchX + sizeRecFBefore,
                touchX + sizeRecFLate
            ),
            PropertyValuesHolder.ofFloat(
                propertyBottom, touchY +
